...
# ==============================================================================
# FishTex General Benchmark — ROCm_bench_general.py
...
# Models requiring F16 KV cache due to architecture constraints.
# Kimi-Linear: n_embd_head_v=72 is not divisible by q8_0 block size (32).
KIMI_LINEAR_MODELS = {
    "Kimi-Linear-48B-Q6_K",
    "Kimi-Linear-48B-Q6_K_L",
    "Kimi-Linear-48B-Q8_0",
...
# ------------------------------------------------------------------------------
# HARDWARE CONFIG — Heavyweight strategy
# ------------------------------------------------------------------------------
...
report_file = f"bench_general_ROCm_{timestamp}.md"
...
with open(report_file, "w") as f:
    f.write("# FishTex General Benchmark — ROCm\n\n")
...
print(f"\n{'='*60}")
print(f"  FishTex General Benchmark — ROCm")
print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M')}")
print(f"  Engine: ROCm | Models: {len(models)}")
...
for model_alias, model_cfg in models.items():
    ...
    print(f"    KV: {'F16 (Kimi-Linear)' if is_kimi else 'q8_0'}")
    print(f"{'='*60}")
