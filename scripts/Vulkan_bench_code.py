...
# ==============================================================================
# FishTex Code Benchmark — Vulkan_bench_code.py
...
#
# Run this AFTER ROCm_bench_code.py has fully completed and cooled down.
...
#   -ctk and -ctv REMOVED ENTIRELY for all models.
#   F16 uncompressed KV for minimum TTFT on short coding prompts.
#   NOTE: Kimi-Linear also requires F16 due to architecture constraint
#   (n_embd_head_v=72 incompatible with q8_0 block size), so the
#   lightweight strategy happens to be correct for both reasons.
#
...
report_file = f"bench_code_Vulkan_{timestamp}.md"


# ------------------------------------------------------------------------------
# CODING PROMPTS — identical to ROCm script for clean comparison
# ------------------------------------------------------------------------------
prompts = [
    {
        "id":   "R_1_Spatial_Join",
...
with open(report_file, "w") as f:
    f.write("# FishTex Code Benchmark — Vulkan\n\n")
...
print(f"\n{'='*60}")
print(f"  FishTex Code Benchmark — Vulkan")
print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M')}")
print(f"  Engine: Vulkan1 (Strix Halo) | KV: F16 | Models: {len(models)}")
...
for model_alias, model_cfg in models.items():
    ...
    try:
        ...
        with open(report_file, "a") as f:
            ...
            f.write(f"- **KV Cache:** F16 uncompressed\n")
